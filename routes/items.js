...
router.get('/category/:categoryId', async (req, res) => {
  try {
    const rows = await query('SELECT * FROM items WHERE category_id = ?', [req.params.categoryId]);
    res.json(rows);
  } catch (err) {
...
router.get('/uncategorized', async (req, res) => {
  try {
    const rows = await query('SELECT * FROM items WHERE category_id IS NULL ORDER BY id');
    res.json(rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
